  ...
  public:
    ...
    std::unique_ptr<SUT> createSut()
    {
        return std::make_unique<SUT>(
            "iox_roudi_test1", mepooConfig, m_managementAllocator, PosixGroup{"iox_roudi_test1"}, PosixGroup{"iox_roudi_test2"});
    }
};
...
TEST_F(MePooSegment_test, SharedMemoryCreationParameter)
{
    ...
    SUT sut{"iox_roudi_test1", mepooConfig, m_managementAllocator, PosixGroup{"iox_roudi_test1"}, PosixGroup{"iox_roudi_test2"}};
